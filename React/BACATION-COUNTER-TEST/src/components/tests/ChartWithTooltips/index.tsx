...
function ExamplePieWithTooltip({
  width,
  height,
...
type AnimatedPieProps<Datum> = ProvidedProps<Datum> & {
  ...
  onClick: (event: React.MouseEvent, d: PieArcDatum<Datum>) => void;
  delay?: number;
};
    ...
    return (
      <g key={key}>
        <animated.path
          ...
          fill={getColor(arc)}
          onClick={(event) => onClick(event, arc)}
          onTouchStart={(event) => onClick(event, arc)}
        />
        {hasSpaceForLabel && (
...
export default withTooltip<BrowserUsage, PieProps>(ExamplePieWithTooltip);
